...
struct PortalStmt {
    ...
    PortalStmt(portalTag tag_, std::unique_ptr<AbstractExecutor> root_, std::shared_ptr<Plan> plan_)
        : tag(tag_), root(std::move(root_)), plan(std::move(plan_)) {}
};
   ...
   public:
    ...
    std::shared_ptr<PortalStmt> start(std::shared_ptr<Plan> plan, Context *context) {
        ...
        if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
            return std::make_shared<PortalStmt>(PORTAL_CMD_UTILITY, std::unique_ptr<AbstractExecutor>(), plan);
        } else if (auto x = std::dynamic_pointer_cast<DDLPlan>(plan)) {
            return std::make_shared<PortalStmt>(PORTAL_MULTI_QUERY, std::unique_ptr<AbstractExecutor>(), plan);
        } else if (auto x = std::dynamic_pointer_cast<DMLPlan>(plan)) {
            switch (x->tag) {
                case T_select: {
                    // std::shared_ptr<ProjectionPlan> p = std::dynamic_pointer_cast<ProjectionPlan>(x->subplan_);
                    std::shared_ptr<Plan> p = x->subplan_;
                    std::unique_ptr<AbstractExecutor> root = convert_plan_executor(p, context);
                    return std::make_shared<PortalStmt>(PORTAL_ONE_SELECT, std::move(root), plan);
                }
    ...
    void run(std::shared_ptr<PortalStmt> portal, QlManager *ql, txn_id_t *txn_id, Context *context) {
        switch (portal->tag) {
            case PORTAL_ONE_SELECT: {
                ql->select_from(std::move(portal->root), context);
                break;
            }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context) {
        if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)) {
            auto ret = std::make_unique<ProjectionExecutor>(convert_plan_executor(x->subplan_, context), x->sel_cols_);
            return ret;
        } else if (auto x = std::dynamic_pointer_cast<ScanPlan>(plan)) {
            if (x->tag == T_SeqScan) {
                return std::make_unique<SeqScanExecutor>(sm_manager_, x->tab_name_, x->conds_, context);
            } else {
                return std::make_unique<IndexScanExecutor>(sm_manager_, x->tab_name_, x->conds_, x->idx_conds_,
            ...
            std::unique_ptr<AbstractExecutor> join =
                std::make_unique<BlockNestedLoopJoinExecutor>(std::move(left), std::move(right), std::move(x->conds_));
            return join;
        } else if (auto x = std::dynamic_pointer_cast<SortPlan>(plan)) {
            auto ret = std::make_unique<SortExecutor>(convert_plan_executor(x->subplan_, context), x->order_bys,
                                                      x->limit_num_);
            return ret;
        } else if (auto x = std::dynamic_pointer_cast<AggPlan>(plan)) {
            return std::make_unique<AggExecutor>(convert_plan_executor(x->subplan_, context), x->sel_col_);
        }
        return nullptr;
    }
};
