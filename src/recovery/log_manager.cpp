...
lsn_t LogManager::gen_log_from_write_set(Transaction* txn, WriteRecord* write_rec) {
    ...
    switch (write_rec->GetWriteType()) {
        case WType::INSERT_TUPLE:
            log = std::make_unique<InsertLogRecord>(txn->get_transaction_id(), write_rec->GetRecord(),
                                                    write_rec->GetRid(), write_rec->GetTableName());
            break;
        case WType::DELETE_TUPLE:

            log = std::make_unique<DeleteLogRecord>(txn->get_transaction_id(), write_rec->GetRecord(),
                                                    write_rec->GetRid(), write_rec->GetTableName());
            break;
        case WType::UPDATE_TUPLE:
            log = std::make_unique<UpdateLogRecord>(txn->get_transaction_id(), write_rec->GetRecord(),
                                                    write_rec->GetNewRecord(), write_rec->GetRid(),
                                                    write_rec->GetTableName());
            break;
        default:
            assert(false);
    }
...
lsn_t LogManager::gen_log_upadte_CLR(txn_id_t tid, lsn_t undo_next, RmRecord& old_value, RmRecord& new_value, Rid& rid,
...
lsn_t LogManager::gen_log_insert_CLR(txn_id_t tid, lsn_t undo_next, RmRecord& insert_value, Rid& rid,
                                     std::string& table_name) {
...
lsn_t LogManager::gen_log_delete_CLR(txn_id_t tid, lsn_t undo_next, RmRecord& delete_value, Rid& rid,
                                     std::string& table_name) {
...
lsn_t LogManager::gen_log_bein(Transaction* txn) {
    latch_.lock();
    auto log_record = std::make_unique<BeginLogRecord>(alloc_lsn(), txn->get_so_far_lsn(), txn->get_transaction_id());
...
lsn_t LogManager::gen_log_commit(Transaction* txn) {
    latch_.lock();
    auto log_record = std::make_unique<CommitLogRecord>(alloc_lsn(), txn->get_so_far_lsn(), txn->get_transaction_id());
    add_log_to_buffer(log_record.get());
    latch_.unlock();
    txn->set_so_far_lsn(log_record->lsn_);
    return log_record->lsn_;
...
lsn_t LogManager::gen_log_abort(Transaction* txn) {
    latch_.lock();
    auto log_record = std::make_unique<AbortLogRecord>(alloc_lsn(), txn->get_so_far_lsn(), txn->get_transaction_id());
    add_log_to_buffer(log_record.get());
    latch_.unlock();
    txn->set_so_far_lsn(log_record->lsn_);
    return log_record->lsn_;
